...
import { Home, User, Code, Briefcase, MessageSquare, BookOpen, Github, Mail, FileText } from "lucide-react"
...
export function CommandPalette({ locale, dict }: CommandPaletteProps) {
  ...
  return (
    <CommandDialog open={open} onOpenChange={setOpen}>
      <CommandInput placeholder={dict.commandPalette?.placeholder || "Type a command or search..."} />
      <CommandList>
        <CommandEmpty>{dict.commandPalette?.noResults || "No results found."}</CommandEmpty>
        ...
        <CommandGroup heading={dict.commandPalette?.actions || "Actions"}>
          <CommandItem onSelect={() => runCommand(() => window.open("https://github.com/Timmi6790", "_blank"))}>
            <Github className="mr-2 h-4 w-4" />
            <span>{dict.commandPalette?.github || "Open GitHub Profile"}</span>
          </CommandItem>
          <CommandItem onSelect={() => runCommand(() => (window.location.href = "mailto:contact@timmi6790.de"))}>
